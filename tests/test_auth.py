...
async def test_register_new_user(client: AsyncClient):
    ...
    response = await client.post("/auth/register", json=_reg_payload("alice", "alice@example.com"))
...
async def test_register_duplicate_username(client: AsyncClient):
    ...
    response = await client.post("/auth/register", json=_reg_payload("bob", "bob2@example.com"))
...
async def test_register_duplicate_email(client: AsyncClient):
    ...
    response = await client.post("/auth/register", json=_reg_payload("carol2", "carol@example.com"))
    assert response.status_code == 409


async def test_register_short_password(client: AsyncClient):
    """Register with password < 8 chars → 422."""
    response = await client.post("/auth/register", json=_reg_payload("dave", "dave@example.com", password="short"))
...
async def test_register_invalid_ed25519_key_size(client: AsyncClient):
    ...
    payload["identity_pub_ed25519"] = base64.b64encode(b"\x01" * 16).decode()  # 16 bytes, invalid
...
async def test_register_invalid_x25519_key_size(client: AsyncClient):
    ...
    payload["identity_pub_x25519"] = base64.b64encode(b"\x02" * 64).decode()  # 64 bytes, invalid
    response = await client.post("/auth/register", json=payload)
    assert response.status_code == 400
...
async def test_protected_endpoint_expired_token(client: AsyncClient):
    ...
    reg = await client.post("/auth/register", json=_reg_payload("grace", "grace@example.com"))
