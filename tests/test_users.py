...
async def test_get_me_200(client: AsyncClient):
    token = await register_and_login(
        client, "usr_alice", "usr_alice@example.com", "password123"
    )
...
async def test_patch_me_valid_display_name_200(client: AsyncClient):
    token = await register_and_login(
        client, "usr_bob", "usr_bob@example.com", "password123"
    )
    resp = await client.patch(
        "/users/me", json={"display_name": "Bobby"}, headers=auth(token)
    )
    assert resp.status_code == 200
...
async def test_patch_me_display_name_too_long_422(client: AsyncClient):
    token = await register_and_login(
        client, "usr_carol", "usr_carol@example.com", "password123"
    )
    long_name = "x" * 65
    resp = await client.patch(
        "/users/me", json={"display_name": long_name}, headers=auth(token)
    )
...
async def test_change_password_correct_current_200(client: AsyncClient):
    token = await register_and_login(
        client, "usr_dave", "usr_dave@example.com", "password123"
    )
    resp = await client.post(
        "/users/me/password",
...
async def test_change_password_wrong_current_401(client: AsyncClient):
    token = await register_and_login(
        client, "usr_eve", "usr_eve@example.com", "password123"
    )
    resp = await client.post(
        "/users/me/password",
